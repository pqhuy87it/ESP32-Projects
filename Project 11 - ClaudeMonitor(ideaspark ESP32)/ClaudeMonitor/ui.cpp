...
static void drawHeaderRight(TFT_eSPI& g, int rssi, unsigned long ago, int batPct) {
    ...
    // batPct < 0 = board không có mạch đo pin (ideaspark 1.9", CrowPanel...).
    // Bỏ hẳn icon + số %, để wifi trôi sát lề phải thay vì in ra "-1%".
    int x = SCREEN_W - 4;
    if (batPct >= 0) {
        char ps[8];
        ...
        x -= 15;   // chỉ chừa đúng bề rộng icon wifi
    }
