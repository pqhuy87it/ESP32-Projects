//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 04_ESP32_TFT_LCD_ILI9341_DS3231_LVGL_EEZ_STUDIO_OpenWeatherMap
//----------------------------------------Including Libraries.
#include <lvgl.h>
#include <TFT_eSPI.h>
#include <WiFi.h>
...
const char* ssid = "MyHouse_2.4G";  //--> Your wifi name
...
String openWeatherMapApiKey = "28c29099075601342f371617f43b2878";

// Replace with your country code and city.
// Find city and country code here : https://openweathermap.org/
String city_code = "Hanoi";
...
// Initialize RTC_DS1302 as "rtc".
const int IO = 23;    // DAT
const int SCLK = 7;  // CLK
const int CE = 1;    // RST
...
void set_initial_display_of_labels() {
  char location[50];
  sprintf(location, "%s, %s", city_name, country_name);
  lv_label_set_text(objects.label_location, location);
...
void get_TimeDate_from_NTP_server() {
  ...
  if(WiFi.status()== WL_CONNECTED){
    ...
    rtc.SetDateTime(RtcDateTime(d_year, d_month, d_day, t_hour, t_minute, t_second));
    ...
    lv_label_set_text(objects.label_info, TimeDate);
    update_UI();
    delay(1000);
...
void get_DateTime() {
  ...
  daysOfTheWeek_Val = now.DayOfWeek();
...
void get_Data_from_OpenWeatherMap() {
  ...
  if(WiFi.status()== WL_CONNECTED){
    String serverPath = "http://api.openweathermap.org/data/2.5/weather?q=" + city_code + "," + country_code + "&units=metric&APPID=" + openWeatherMapApiKey;
...
void setup() {
  ...
  Serial.println();
  Serial.println("------------");
  Serial.println("Starting the DS1302 RTC module.");
